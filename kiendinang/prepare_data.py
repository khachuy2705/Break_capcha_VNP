...
def crawl_images():
	...
	for i in range (1, IMAGE_TOTAL):
		...
		with open(file_path, 'wb') as f:
			response = requests.get(url)
			png_recovered = base64.decodebytes(response.content)
			if response.ok: f.write(png_recovered)
			time.sleep(.5)
...
def reduce_noise(file_path):
	...
	dst = cv2.fastNlMeansDenoisingColored(img,None,9,20,7,51)
...
def reduce_noise_dir(directory):
	...
	for file_path in list_file:
		...
		# dst = cv2.fastNlMeansDenoisingColored(img,None,7,21,7,21)
		cv2.imwrite(file_path, dst)
		img = Image.open(file_path).convert('L')
		img = img.point(lambda x: 0 if x<128 else 255, '1')
		img.save(file_path)
...
if __name__=='__main__':
	# crawl_images()
	reduce_noise("D:/Lab_Zone/Python/Break_capcha_VNP/captcha_storaged/_"+str(AB)+"_.png")
